...
- `POLL_QUESTIONS` — id, eyebrow number, headline (HTML allowed for `<sub>`, `<b>`, etc.), supporting copy, option list, short labels for the reveal chart.
- `HOOK_HEADLINES` — rotating hook lines. Switch by URL: `?hook=A|B|C|D`.
- `BENCHMARK` — the dark ZEF marker card on the reveal chart. Point it at any question via `question_id`. Set to `None` to hide it.
- `LEAD_*` — copy for the email-capture screen.
- `LEAD_ROLES` — the role chips on the lead form.

**Important:** if you add, remove, or rename a question `id`, also update `QUESTION_IDS` at the top of `apps_script.gs` and redeploy (Deploy → Manage deployments → edit → New version).
...
- All tap targets are ≥ 44 px; lock-and-advance means no Next buttons.
...
| `POST` | `/api/poll` | `{ sessionId, submittedAt?, source?, answers: { q1, q2, q3, q4 } }`. |
| `POST` | `/api/lead` | `{ sessionId, email, role?, consent, submittedAt? }`. |
| `GET` | `/api/healthz` | `{ ok, mode, questions }`. |

## Deploy

Anything that runs `uvicorn main:app` works — Render, Railway, Fly.io, a small VM. Set the environment variables (`POLL_STORAGE_MODE=live`, `APPS_SCRIPT_URL=…`) in the host's dashboard instead of `.env.local`.

## Demo hotkeys (desktop only)

- `d` — jump to REVEAL with placeholder answers.
- `h` — return to the hook (clears answers).
- `←` / `→` — step through scenes (useful for screenshots).
